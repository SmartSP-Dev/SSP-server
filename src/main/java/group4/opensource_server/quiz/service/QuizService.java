...
public class QuizService {
    ...
    public JSONObject generateQuizzesFromText(String text, String keyword, QuestionType questionType) {
        ...
        // 시스템 프롬프트와 사용자 프롬프트 구분
        // 시스템 프롬프트는 callOpenAiApi에서 별도 전달
        ...
        promptBuilder.append("아래 텍스트를 참고하여, 키워드 '")
                ...
                .append("내용 전체를 골고루 반영하여 출제해 주세요.\n")
        ...
        switch (questionType) {
            ...
            case OX:
                ...
                break;
            case FILL_BLANK:
                promptBuilder.append("출력 형식 예시: { \"type\": \"fill_in_the_blank\", \"question\": \"문장 ___\", ")
                        .append("\"correct_answer\": \"정답\", \"incorrect_answers\": [\"오답1\", \"오답2\", \"오답3\"] }\n");
                break;
            default:
    ...
    @Transactional
    public Quiz createQuizWithQuestions(JSONObject openAiResponse, String title, String keyword, QuestionType questionType, Long userId) {
        // 1. Quiz 저장
        ...
        for (int i = 0; i < quizzes.length(); i++) {
            ...
            QuizQuestion question = QuizQuestion.builder()
                    .quiz(quiz)
                    .questionTitle(quizObject.getString("question"))
                    .questionContent("") // 지금은 비워둬도 됨
                    .quizNumber(i + 1)  // 1번부터 번호 부여
                    .correctAnswer(quizObject.getString("correct_answer"))
                    .questionType(questionType) // Quiz랑 같은 타입
                    .build();
